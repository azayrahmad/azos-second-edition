...
You can also disable this warning by passing {iframes: {ignoreCrossOrigin: true}} to $Window.`),
    ...
    function handle_pointer_activation(event) {
      // If the target is a <select> element, we want to let the browser handle it natively.
    ...
    var update_drag = (e) => {
      ...
      const parentRect = $w[0].offsetParent.getBoundingClientRect();
      $w.css({
    ...
    $w.$titlebar.on("pointerdown", (e) => {
      ...
      const parentRect = $w[0].offsetParent.getBoundingClientRect();
      drag_offset_x = e.clientX - $w.position().left - parentRect.left;
      drag_offset_y = e.clientY - $w.position().top - parentRect.top;
    ...
    $G.on("pointerup pointercancel", (e) => {
      const pointerId = e.pointerId ?? e.originalEvent?.pointerId; // originalEvent doesn't exist for triggerHandler()
      if (pointerId !== drag_pointer_id && pointerId !== undefined) {
        return;
      } // (allowing synthetic events to affect the drag without pointerId)
      ...
      handle_positions.forEach(([y_axis, x_axis]) => {
        ...
        $handle.on("pointerdown", (e) => {
          ...
          try {
            ...
            console.warn(
              "Failed to capture pointer for resize handle drag:",
              error,
            );
          }
        ...
        function handle_pointermove(/** @type {JQuery.TriggeredEvent} */ e) {
          const pointerId = e.pointerId ?? e.originalEvent?.pointerId; // originalEvent doesn't exist for triggerHandler()
          if (pointerId !== resize_pointer_id && pointerId !== undefined) {
            return;
          } // (allowing synthetic events to affect the drag without pointerId)
          resize_pointer_x = e.clientX;
          resize_pointer_y = e.clientY;
    ...
    $w.title = /** @type {titleMethodOverloads} */ (
      (title) => {
