((exports) => {
    ...
    function MenuBar(menus) {
        ...
        function handleKeyDown(e) {
            ...
            const parent_item_el = active_menu_popup_el ? top_level_menus.find(m => m.menu_popup === active_menu_popup.parentMenuPopup)?.menu_button_el : undefined;
            ...
            switch (e.key) {
                ...
                case "ArrowRight":
                    ...
                    if (
                        ...
                        let new_index = top_level_menu_index === -1 ? (cycle_dir === 1 ? 0 : top_level_menus.length - 1) : (top_level_menu_index + cycle_dir + top_level_menus.length) % top_level_menus.length;
                ...
                case "Escape":
                    if (active_menu_popup) {
                        ...
                        menus_el.closest(".window")?.dispatchEvent(new CustomEvent("refocus-window"));
                        e.preventDefault();
                    }
                    break;
                case "Alt":
                ...
                default:
                    ...
                    const item_els = active_menu_popup ? [...menu_popup_el.querySelectorAll(".menu-item")] : top_level_menus.map(m => m.menu_button_el);
                    const item_els_by_access_key = {};
                    for (const item_el of item_els) {
                        const access_key_el = item_el.querySelector(".menu-hotkey");
                        const access_key = (access_key_el ? access_key_el.textContent : (item_el.querySelector(".menu-item-label") ?? item_el).textContent[0]).toLowerCase();
                    ...
                    if (matching_item_els.length) {
                        if (matching_item_els.length === 1) {
                            ...
                            const top_level_menu = top_level_menus.find(m => m.menu_button_el === menu_item_el);
                            if (top_level_menu) {
        ...
        const make_menu_button = (menus_key, menu_items) => {
            ...
            function open_top_level_menu(type = "other") {
                ...
                if (new_index === top_level_menu_index && menu_button_el.getAttribute("aria-expanded") === "true") return;
                close_menus();
                ...
                if (!menu_popup_el.parentElement) document.body.appendChild(menu_popup_el);
                top_level_highlight(menus_key);
        ...
        function set_keyboard_scope(...elements) {
            keyboard_scope_elements.forEach(el => el.removeEventListener("keydown", keyboard_scope_keydown));
            keyboard_scope_elements = elements;
            keyboard_scope_elements.forEach(el => el.addEventListener("keydown", keyboard_scope_keydown));
        }
        ...
        function keyboard_scope_keydown(e) {
            ...
            if (e.altKey && !e.shiftKey && !e.ctrlKey && !e.metaKey) {
                const menu = top_level_menus.find(m => m.access_key?.toLowerCase() === e.key.toLowerCase());
                if (menu) {
                    e.preventDefault();
