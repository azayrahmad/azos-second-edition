((exports) => {
    ...
    const { E, uid, get_new_menu_z_index, get_direction, is_disabled } = window.os_gui_utils;
        ...
        const add_menu_item = (parent_element, item, item_index) => {
            ...
            if (item === MENU_DIVIDER) {
                ...
                item_el.addEventListener("pointerleave", (event) => {
                    if (menu_popup_el.style.display !== "none" && event.pointerType !== "touch") {
                ...
                if (item.submenu) {
                    ...
                    const submenu_popup = new MenuPopup(item.submenu, { ...options, parentMenuPopup: this });
