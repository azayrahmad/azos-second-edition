...
export class CalculatorApp extends Application {
    ...
    _setMode(newMode) {
        ...
        if (this.mode === 'scientific') {
            this.win.resizeTo(480, 280);
        } else {
            this.win.resizeTo(260, 280);
        }
        this._renderButtons();
    ...
    _renderButtons() {
        ...
        const layout = this.mode === 'standard' ? this._getStandardLayout() : this._getScientificLayout();
    ...
    _getStandardLayout() {
        return [
            [{label: '', style: 'width: 25px;', key: 'noop'}, {label: 'Backspace'}, {label: 'CE'}, {label: 'C'}],
            [{label: 'MC'}, {label: '7'}, {label: '8'}, {label: '9'}, {label: '/', class: 'red-text'}, {label: 'sqrt', class: 'blue-text'}],
            [{label: 'MR'}, {label: '4'}, {label: '5'}, {label: '6'}, {label: '*', class: 'red-text'}, {label: '%', class: 'blue-text'}],
            [{label: 'MS'}, {label: '1'}, {label: '2'}, {label: '3'}, {label: '-', class: 'red-text'}, {label: '1/x', class: 'blue-text'}],
            [{label: 'M+'}, {label: '0'}, {label: '+/-'}, {label: '.'}, {label: '+', class: 'red-text'}, {label: '=', class: 'red-text'}]
        ];
    }

    _getScientificLayout() {
        return [
            [{label: 'Sta', class: 'blue-text', style: 'width: 40px;'}, {label: 'F-E'}, {label: '('}, {label: ')'}, {label: 'MC'}, {label: '7'}, {label: '8'}, {label: '9'}, {label: '/', class: 'red-text'}, {label: 'Mod'}, {label: 'And'}],
            [{label: 'Ave', class: 'blue-text', style: 'width: 40px;'}, {label: 'dms'}, {label: 'Exp'}, {label: 'ln'}, {label: 'MR'}, {label: '4'}, {label: '5'}, {label: '6'}, {label: '*', class: 'red-text'}, {label: 'Or'}, {label: 'Xor'}],
            [{label: 'Sum', class: 'blue-text', style: 'width: 40px;'}, {label: 'sin'}, {label: 'x^y'}, {label: 'log'}, {label: 'MS'}, {label: '1'}, {label: '2'}, {label: '3'}, {label: '-', class: 'red-text'}, {label: 'Lsh'}, {label: 'Not'}],
            [{label: 's', class: 'blue-text', style: 'width: 40px;'}, {label: 'cos'}, {label: 'x^3'}, {label: 'n!'}, {label: 'M+'}, {label: '0'}, {label: '+/-'}, {label: '.'}, {label: '+', class: 'red-text'}, {label: '=', class: 'red-text'}, {label: 'Int'}],
            [{label: 'Dat', class: 'blue-text', style: 'width: 40px;'}, {label: 'tan'}, {label: 'x^2'}, {label: '1/x'}, {label: 'pi'}, {label: 'A'}, {label: 'B'}, {label: 'C'}, {label: 'D'}, {label: 'E'}, {label: 'F'}]
        ];
    }
    ...
    _renderScientificControls() {
        ...
        `;
        const displayContainer = this.win.$content.find('.calculator-display-container')[0];
        displayContainer.insertAdjacentHTML('afterend', controlsHTML);

        this.win.$content.find('input[name="number-system"]').forEach(radio => {
            radio.addEventListener('change', (e) => this._handleBaseChange(parseInt(e.target.value)));
        });
        this.win.$content.find('input[name="angle-measure"]').forEach(radio => {
            radio.addEventListener('change', (e) => this.logic.setAngleUnit(e.target.value));
        });
    }
    ...
    _updateHexButtonState() {
        const hexButtons = this.win.$content.find('[data-key="A"], [data-key="B"], [data-key="C"], [data-key="D"], [data-key="E"], [data-key="F"]');
    ...
    _attachButtonListeners() {
        ...
        Array.from(buttons).forEach(button => {
            button.addEventListener('click', () => this._handleButtonClick(button.dataset.key));
        });
    }
    ...
    _handleButtonClick(key) {
        if (!isNaN(parseInt(key, this.logic.base))) {
            ...
            switch (key) {
                case '.': this.logic.inputDecimal(); break;
                case '+':
                case '-':
                case '*':
                case '/':
                ...
                case 'Xor':
                case 'Lsh':
                case 'Mod': this.logic.performOperation(key); break;
                case '=': this.logic.equals(); break;
                case 'C': this.logic.clearAll(); break;
                case 'CE': this.logic.clearEntry(); break;
                case 'Backspace': this.logic.backspace(); break;
                case '+/-': this.logic.toggleSign(); break;
                case 'sqrt': this.logic.squareRoot(); break;
                case '%': this.logic.percentage(); break;
                case '1/x': this.logic.reciprocal(); break;
                case 'n!': this.logic.factorial(); break;
                case 'sin': this.logic.sin(); break;
                case 'cos': this.logic.cos(); break;
                case 'tan': this.logic.tan(); break;
                case 'log': this.logic.log(); break;
                case 'ln': this.logic.ln(); break;
                case 'pi': this.logic.pi(); break;
                case 'x^2': this.logic.x_squared(); break;
                case 'x^3': this.logic.x_cubed(); break;
                case 'Not': this.logic.not(); break;
                case 'Int': this.logic.int(); break;
                case 'MC': this.logic.memoryClear(); break;
                case 'MR': this.logic.memoryRecall(); break;
                case 'MS': this.logic.memoryStore(); break;
                case 'M+': this.logic.memoryAdd(); break;
            }
        }
        this._updateDisplay();
    }
    ...
    _handleKeyPress(key) {
        if (!isNaN(parseInt(key, this.logic.base))) {
            ...
            switch (key) {
                ...
                case '-':
                case '*':
                case '/': this.logic.performOperation(key); break;
                case 'Enter':
                case '=': this.logic.equals(); break;
                case 'Escape': this.logic.clearAll(); break;
                case 'Backspace': this.logic.backspace(); break;
            }
        }
        this._updateDisplay();
