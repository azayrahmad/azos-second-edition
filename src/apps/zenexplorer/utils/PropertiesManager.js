...
import { formatPathForDisplay, getDisplayName, joinPath, getPathName } from "./PathUtils.js";
...
export class PropertiesManager {
    ...
    static async _showMultipleProperties(items) {
        ...
        const parentPathInternal = items[0].path.substring(0, lastSlashIndex) || "/";
        const location = `All in ${formatPathForDisplay(parentPathInternal)}`;
        const size = this._formatSize(totalSize);

        const content = this._createPropertiesUI({
            iconUrl: ICONS.file[32], // Generic multiple files icon
            name,
            type,
            location,
            size
