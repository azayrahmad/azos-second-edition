...
function findExplorerInstanceByWindow(windowElement) {
...
export class ExplorerApp extends Application {
  ...
  createExplorerIcon(item) {
    ...
    if (!item.isStatic) {
      this.configureIconForDragging(iconDiv, item);
    }

    return iconDiv;
  }

  configureIconForDragging(icon, item) {
    ...
    const handleDragStart = (e) => {
      ...
      this.iconManager.handleIconMouseDown(e, icon);
    ...
    const handleDragMove = (e) => {
      ...
      if (Math.abs(clientX - dragStartX) > 5 || Math.abs(clientY - dragStartY) > 5) {
        if (!wasDragged) {
          ...
          this.iconManager.selectedIcons.forEach((selectedIcon) => {
    ...
    const handleDragEnd = (e) => {
      ...
      if (wasDragged) {
        ...
        const targetIcon = dropTarget ? dropTarget.closest(".explorer-icon, .desktop-icon") : null;
        const targetDesktop = dropTarget ? dropTarget.closest(".desktop") : null;

        if (targetIcon && !this.iconManager.selectedIcons.has(targetIcon)) {
        ...
        if (destinationPath) {
          const draggedItems = [...this.iconManager.selectedIcons]
            .map((icon) => this.getItemFromIcon(icon))
            .filter(Boolean);
          ...
          if (isInvalidMove) {
            ...
            if (areAllFilesDraggable) {
              const fileIds = draggedItems.map(item => item.id);
              moveFiles(fileIds, destinationPath);
              dropHandled = true;
            }
          }
        }
