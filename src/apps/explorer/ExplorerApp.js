...
export class ExplorerApp extends Application {
    ...
    _createWindow() {
        ...
        this._addAnimatedIcon(menuBar.element);
    ...
    _addAnimatedIcon(menuBarElement) {
        const animatedIconContainer = document.createElement('div');
        animatedIconContainer.className = 'animated-icon-container';
        ...
        menuBarElement.appendChild(animatedIconContainer);

        let frame = 0;
        const totalFrames = 41;
        const frameHeight = 38;

        animatedIconContainer.addEventListener('pointerenter', () => {
            if (this.animationInterval) return;
            this.animationInterval = setInterval(() => {
                frame = (frame + 1) % totalFrames;
                animatedIcon.style.backgroundPosition = `0 -${frame * frameHeight}px`;
            }, 100);
        });

        animatedIconContainer.addEventListener('pointerleave', () => {
            clearInterval(this.animationInterval);
            this.animationInterval = null;
            frame = 0;
            animatedIcon.style.backgroundPosition = `0 0`;
        });
    }
