...
export class NotepadApp extends Application {
  constructor(fileOrConfig) {
    const config = fileOrConfig.app
      ? { ...fileOrConfig, ...apps.find((a) => a.id === fileOrConfig.app) }
      : fileOrConfig;
    super(config);
  ...
  async openFile() {
    ...
    input.type = "file";
    input.accept = languages
      .flatMap((lang) => lang.extensions.map((ext) => `.${ext}`))
      .join(",");
    input.onchange = (e) => {
    ...
    if (window.showSaveFilePicker) {
      try {
        const fileTypes = languages.map((lang) => ({
          ...
          accept: {
            [lang.mimeType || "text/plain"]: lang.extensions.map(
              (ext) => `.${ext}`,
            ),
          },
        }));
  ...
  getInlineStyledHTML() {
    ...
    tempPre.style.cssText =
      "background-color: #fafafa; padding: 12px; font-family: monospace; white-space: pre; overflow-x: auto;";
  ...
  copyFormattedCode() {
    try {
      ...
      window.getSelection().removeAllRanges();
      this.editor.statusText.textContent = successful
        ? "✓ Copied to clipboard!"
        : "Copy failed!";
    } catch (err) {
  ...
  formatCode() {
    ...
    const language = languages.find(
      (lang) => lang.id === this.editor.currentLanguage,
    );
