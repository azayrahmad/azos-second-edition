(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        define(['./utils', './MenuPopup'], factory);
    } else if (typeof module === 'object' && module.exports) {
        // Node. Does not work with strict CommonJS, but
        // only CommonJS-like environments that support module.exports,
        // like Node.
        module.exports = factory(require('./utils'), require('./MenuPopup'));
    } else {
        // Browser globals (root is window)
        const osGui = factory(root.osGuiUtils, root.MenuPopup);
        root.MenuBar = osGui.MenuBar;
        root.AccessKeys = osGui.AccessKeys;
    }
}(typeof self !== 'undefined' ? self : this, function (osGuiUtils, MenuPopupModule) {
    'use strict';

    const { E, uid, get_new_menu_z_index, get_direction } = osGuiUtils;
    const { MenuPopup } = MenuPopupModule;
    ...
    function MenuBar(menus, config = {}) {
        if (!(this instanceof MenuBar)) {
            return new MenuBar(menus, config);
        }
        ...
        const make_menu_button = (menus_key, menu_items) => {
            ...
            const open_top_level_menu = (type = "other") => {
