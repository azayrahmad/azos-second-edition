(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        define(['./utils', './MenuBar'], factory);
    } else if (typeof module === 'object' && module.exports) {
        // Node. Does not work with strict CommonJS, but
        // only CommonJS-like environments that support module.exports,
        // like Node.
        module.exports = factory(require('./utils'), require('./MenuBar'));
    } else {
        // Browser globals (root is window)
        const osGui = factory(root.osGuiUtils, root.MenuBar);
        root.MenuPopup = osGui.MenuPopup;
        root.MENU_DIVIDER = osGui.MENU_DIVIDER;
    }
}(typeof self !== 'undefined' ? self : this, function (osGuiUtils, MenuBar) {
    'use strict';

    const { E, uid, get_new_menu_z_index, get_direction, is_disabled } = osGuiUtils;
    const { AccessKeys } = MenuBar;
            ...
            if (item === MENU_DIVIDER) {
                ...
                if (item.submenu) {
                    ...
                    open_submenu = (highlight_first = true) => {
                        if (typeof this.config.playSound === "function") {
                            this.config.playSound("MenuPopup");
                        }
                        ...
                        const rect = submenu_popup_el.querySelector(".menu-popup").getBoundingClientRect();

                        let final_x = (get_direction() === "rtl" ? rect.left - rect.width : rect.right) + window.scrollX;
                        ...
                        if (get_direction() === "rtl") {
                            ...
                            if (final_x + rect.width > innerWidth) {
                                final_x = rect.left - rect.width;
                                from_left = true;
                            }
                        }
                        if (final_y + rect.height > innerHeight) {
                            final_y = Math.max(0, innerHeight - rect.height);
                        }
                        ...
                        setTimeout(() => {
                            submenu_popup_el.style.setProperty("--width", `${rect.width}px`);
                            submenu_popup_el.style.setProperty("--height", `${rect.height}px`);
                ...
                const item_action = () => {
                    ...
                    if (typeof this.config.playSound === "function") {
                        this.config.playSound("MenuCommand");
                    }
                    if (item.checkbox) {
