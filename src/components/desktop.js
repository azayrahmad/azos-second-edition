...
function configureIcon(icon, app, filePath = null, { iconManager }) {
  ...
  const itemData = filePath && typeof filePath === 'object' ? filePath : { id: icon.getAttribute("data-file-id") || icon.getAttribute("data-icon-id"), ...app };

  // Only user-created files should be draggable
  if (itemData.id && (itemData.id.startsWith('dropped-') || itemData.id.startsWith('file-'))) {
    configureDraggableIcon(icon, itemData, iconManager, getItemFromIcon, deleteDroppedFile);
  } else {
      // Logic for non-draggable icons (e.g., system icons)
